...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($username) || empty($password)) {
        ...
        $stmt = $conn->prepare("SELECT staff_id, username, password FROM staffs WHERE username = ? AND password = ?");
        ...
        if ($stmt->num_rows === 1) {
            $stmt->bind_result($staffId, $dbUsername, $dbPassword);
            $stmt->fetch();
...
<body>
    ...
    <p>Don't have an account? <a href="staff_signup.php">Sign Up</a></p>
</body>
</html>
