...
<head>
    ...
    <title>New Transaction Form</title>
</head>
<body>
...
if (!isset($_SESSION['staff_id'])) {
    header("Location: staff_login.php"); // Redirect to your login page
    exit();
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $customerSql = "INSERT INTO customers (firstname, lastname) VALUES ('$firstname', '$lastname')";
    if ($conn->query($customerSql) === TRUE) {
        // Get the last inserted customer_id
        $lastCustomerId = $conn->insert_id;

        // Insert data into the transactions table
        $transactionSql = "INSERT INTO transactions (customer_id, staff_id, service_id, timeOfDropOff, numberofLoads, numberofClothing, totalCost,paymentStatus, tenderedAmount,changeAmount, detergent_id,numberOfDetergent, fabric_softener_id,numberOfFabricSoft)
                VALUES ('$lastCustomerId', '$staff_id', '$service_id', '$timeOfDropOff', '$numberofLoads', '$numberofClothing', '$totalCost','$paymentStatus','$tenderedAmount','$changeAmount', '$detergentDetails','$numberOfDetergent', '$fabricSoftenerDetails','$numberOfFabricSoft')";
        ...
        if ($conn->query($transactionSql) === TRUE) {
            ...
            if ($conn->query($trackingSql) === TRUE) {
                header('Location: success_page.php?trackingNumber=' . $trackingNumber);
            } else {
...
<h2>Transaction Form for New Customer</h2>
